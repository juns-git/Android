...
fun LedgerScreen(viewModel: AppViewModel, innerPadding: PaddingValues) {
    ...
    val dummyPendingChildren = listOf("child_uid_홍길동", "child_uid_김철수")
    ...
    Column(modifier = Modifier.fillMaxSize().padding(innerPadding)) {
        ...
        TabRow(selectedTabIndex = selectedTab) {
            Tab(
                ...
                text = { Text("대기 중인 요청 (${activePending.size + dummyPendingChildren.size})") }
            )
            Tab(
        ...
        when (selectedTab) {
            0 -> PendingTab(
                role = role,
                pendingChildren = dummyPendingChildren,
            ...
            1 -> CompletedTab(tradeRequests = completed)
        }
    }
    ...
    fillDialogTarget?.let { req ->
        FillDialog(
            request = req,
            onConfirm = { filledPrice, filledQty ->
                viewModel.fillTradeRequest(req.requestId, filledPrice, filledQty)
    ...
    if (role == UserRole.PARENT) {
        LazyColumn(
            ...
            if (pendingChildren.isNotEmpty()) {
                ...
                items(pendingChildren) { childUid ->
                    PendingChildCard(childUid, onApprove = { onApproveChild(childUid) },
...
@Composable
private fun CompletedTab(tradeRequests: List<TradeRequest>) {
    if (tradeRequests.isEmpty()) {
...
@Composable
private fun PendingChildCard(childUid: String, onApprove: () -> Unit, onReject: () -> Unit) {
    Card(
        ...
        Column(Modifier.padding(12.dp)) {
            ...
            Text("${childUid} 님이 가족 연결을 요청했습니다.",
                color = MaterialTheme.colorScheme.onSecondaryContainer)
...
@Composable
private fun CompletedCard(request: TradeRequest) {
...
private fun TradeRequestHeader(request: TradeRequest) {
    ...
    Row(
        ...
        TradeStatusChip(status = request.status)
    }
}

@Composable
private fun TradeStatusChip(status: TradeStatus) {
...
private fun FillDialog(
    request: TradeRequest,
    onConfirm: (filledPrice: Long, filledQuantity: Int) -> Unit,
    onDismiss: () -> Unit
) {
    ...
    AlertDialog(
        ...
        confirmButton = {
            Button(
                onClick = {
                    onConfirm(priceText.toLongOrNull() ?: 0L, qtyText.toIntOrNull() ?: 0)
                },
                enabled = (priceText.isNotBlank() && (qtyText.toIntOrNull() ?: 0) > 0)
